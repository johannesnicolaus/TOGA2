...
@click.argument(
    ...
    metavar='INIT_LOSS_SUMMARY'
)
@click.argument(
    'query_bed_file',
    type=click.File('r', lazy=True),
    metavar='FINAL_BED_FILE'
)
@click.argument(
    'final_loss_summary',
    type=click.File('w', lazy=True),
    metavar='FINAL_LOSS_SUMMARY'
)
@click.option(
    '--paralogs',
    '-p',
...
class LossFileFilter(CommandLineManager):
    ...
    """
    __slots__ = (
        'loss_in', 'bed_file', 'loss_out', 'paralogs', 'ppgenes'
    )
    ...
    def run(self) -> None:
        ...
        for i, line in enumerate(self.loss_in, start=1):
            ...
            if data[0] != PROJECTION:
                ...
                continue
            if data[1] not in projections and base_proj_name(data[1]) not in projections:
                continue
